...
- Production stack DEPLOYED (2026-08-18): repo at `~/apps/verder` (synced via rsync, no GitHub key on homelab), data under `/srv/verder/{vault,backups,scans-inbox}`, stack via `docker compose --env-file .env.prod -f docker-compose.prod.yml`, nightly cron 03:30 (BACKUP_DIR=/mnt/nas-download/verder-backups, off-box on the NAS since 2026-08-18). Secrets in `~/apps/verder/.env.prod` and `~/apps/verder/secrets/role-passwords` (never commit). App reachable ONLY via https://verder.vanderpoel.pro (see Public access below). Financial registry (sub-project 2) deployed 2026-08-19 (migrations 0007–0009, `registry.mine` cron live). Tasks + milestones (sub-project 3) deployed 2026-08-19 (migrations 0010–0011; task mining rides the suggest.entry job).
