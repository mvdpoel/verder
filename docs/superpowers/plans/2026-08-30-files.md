...
**Spec:** `docs/superpowers/specs/2026-08-30-files-design.md`

> **Corrections applied during execution.** Seven pieces of code in this plan could not run as written and were corrected against the real codebase; `.superpowers/sdd/2026-08-30-files/progress.md` on the branch records each with its ruling. The ones most worth knowing: `db.execute()` returns a pg `QueryResult`, so a test destructures `const { rows: [row] } = ...`; `'\\s+'` needs the doubled backslash because `'\s+'` in a JS template literal silently drops it and Postgres then matches a literal "s"; the `zipEntryName` sanitiser below contains **raw control bytes** instead of `/[\x00-\x1f\x7f]/g` and does not parse; a nested correlated subquery inside a grouped query is rejected by Postgres and became a `LEFT JOIN`; and `documents.tree`/`browse`/`ruleWhere` must fold a soort key through ONE shared constant, not three copies. — read it first; this plan argues from it.
