...
- **Evidence tables are append-only** and every evidence mutation appends a `ledger_events` row in the same transaction. This change adds one additive migration (0020, `ALTER TYPE tx_source ADD VALUE 'abn-xls'` — found in Task 4, see the note there) and must not weaken that.
...
- Modify: `packages/db/src/schema.ts` *(deviation — see Step 8)*
- Add: `packages/db/drizzle/0020_abn_xls_tx_source.sql` *(deviation — see Step 8)*
...
- [x] **Step 1: Write the failing detection test**
...
- [x] **Step 3: Teach `detectSource` the binary containers**
...
- [x] **Step 6: Write the failing import test**
...
> **Deviation found while executing Step 8.** `transactions.source` is the
> Postgres enum `tx_source`, so `abn-xls` could be detected and parsed but not
> stored: the insert failed with `invalid input value for enum tx_source`. The
> plan's "no migration" claim was checked against `document_texts.extractor`
> (a text column) and missed this one. Fixed by adding the value to
> `txSourceEnum` in `packages/db/src/schema.ts` and generating migration
> `0020_abn_xls_tx_source.sql` (`ALTER TYPE ... ADD VALUE`, additive only — it
> rewrites no row, so the append-only guarantee is untouched). Task 9's runbook
> must therefore run `pnpm --filter @verder/db migrate` before deploying.
>
> Two smaller corrections in the same step: the import test's fixtures are made
> per-run unique (the shared dev DB is never truncated, so a fixed fixture
> would insert 5 rows on the first run and 0 on every run after), and an
> `.xlsx` ingest case was added because nothing else gave `mimeFor`'s
> bytes-over-source rule teeth.
...
```
Spreadsheet support deployed 2026-08-20 (migration 0020, additive enum value): ABN AMRO's "Excel"
