...
**Call site fix:** `registry-import.ts:51` passes `buf.subarray(0, 1024)`. OLE2
magic sits in the first 8 bytes and is fine, but `xl/workbook.xml` is not
reliably within the first kilobyte of an OOXML file. The head slice widens to
8 KB. `StatementSource` gains `"abn-xls"`, and `PARSERS` a matching entry.
